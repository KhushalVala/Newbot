...
from curl_cffi import requests # 👈 Asali Chrome 120 jaisa bypass engine

# --- BOT CONFIG ---
# Naya aur fresh token add kar diya gaya hai
...
def run_assassin_bypass(short_url, chat_id, msg_id):
    def update_status(text):
        try: bot.edit_message_text(f"⚡ **Assassin Engine v24**\n`{text}`", chat_id, msg_id, parse_mode="Markdown")
        except: pass
    ...
    # Create a Chrome-impersonating session (Bypasses Cloudflare natively)
    ...
    try:
        ...
        if res.status_code in [403, 503]:
            print(f"[-] Debug: Cloudflare ne IP block kar di hai! Status: {res.status_code}")
            return f"[-] Error: Cloudflare Blocked (Status {res.status_code})"
        time.sleep(1)
        ...
        for attempt in range(15):
            ...
            if data.get("url"):
                ...
                print(f"[-] Debug: Unexpected JSON - {data}")
                break
        ...
        if final_raw_link:
            ...
            # Ye line bata degi ki link kyu nahi mili
            error_msg = f"[-] Debug: Server timeout or failed to resolve. Last API response: {response.text[:100]}"
            print(error_msg)
            return error_msg
...
if __name__ == "__main__":
    ...
    print("🔥 THE MODS KING BOT IS LIVE (V24 + NEW TOKEN) 🔥")
