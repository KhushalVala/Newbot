...
def auto_delete(chat_id, user_msg_id, bot_msg_id):
    """60 seconds baad messages delete karne ke liye"""
    time.sleep(60)
    try:
        bot.delete_message(chat_id, user_msg_id)
        bot.delete_message(chat_id, bot_msg_id)
        print(f"[*] Chat cleared for {chat_id}")
    except Exception as e:
        print(f"[-] Delete error: {e}")

# ==========================================================
# 💎 TERA ORIGINAL LOGIC (FIXED REGEX)
# ==========================================================
...
def bypass_yorurl_short(short_url, scraper):
    # Fixed YorURL regex to be more flexible
...
def run_assassin_bypass(short_url, chat_id, msg_id):
    ...
    # FIXED: Ab ye gx ho ya normal ID, sab uthayega
    match = re.search(r'go\.urlking\.site/([a-zA-Z0-9]+)', short_url)
    if not match: return "[-] Error: URL ID format valid nahi hai!"
    ...
    try:
        ...
        for attempt in range(15):
            ...
            except: return "[-] Server Error: JSON decode failed."
...
if __name__ == "__main__":
    ...
    print("🔥 TMK ASSASSIN BOT RUNNING 🔥")
